...
export class IngestionWorkflow extends WorkflowEntrypoint<Env, IngestionParams> {
  async run(event: WorkflowEvent<IngestionParams>, step: WorkflowStep) {
    ...
    if (mode === "makes-only") {
      await step.do("log-ingestion-run", () => this._logRun(startedAt, mode, targetMake, "completed", filteredMakes.length, filteredMakes.length));
      return { ok: true };
    }
    ...
    for (const make of filteredMakes) {
      ...
      // Batch models for recall ingestion
      ...
      for (let batchIdx = 0; batchIdx < batches.length; batchIdx++) {
        ...
        const saved = await step.do(`recalls-${makeSlug}-batch-${batchIdx}`, async () => {
          let count = 0;
          for (const model of batch) {
            for (let year = startYear; year <= endYear; year++) {
              try {
                ...
                ).bind(model.modelId, year, now, now).run();
                ...
                console.error(`Failed ${make.Make_Name} ${model.modelName} ${year}:`, err);
              }
            }
          }
          return count;
    ...
    await step.do("log-ingestion-run", () =>
      this._logRun(startedAt, mode, targetMake, "completed", totalRecordsSaved, totalRecordsSaved)
    );
  ...
  private async _logRun(
    ...
    await this.env.DB.prepare(
      `INSERT INTO ingestion_logs (run_type, target_make, status, records_found, records_saved, started_at, completed_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(runType, targetMake ?? null, status, recordsFound, recordsSaved, startedAt, new Date().toISOString()).run();
  }
}
