...
> {
  async run(event: WorkflowEvent<EnrichmentParams>, step: WorkflowStep) {
    ...
    while (true) {
      // Fetch unenriched recalls for this batch
      const rows = await step.do(
        `fetch-unenriched-batch-${offset}`,
        async () => {
          let query: string;
          let params: unknown[];
      ...
      for (let chunkIdx = 0; chunkIdx < chunks.length; chunkIdx++) {
        ...
        const chunkEnriched = await step.do(
          `enrich-batch-${offset}-chunk-${chunkIdx}`,
          async () => {
            const results = await Promise.allSettled(
              chunk.map(async (row) => {
                ...
                if (!enriched) {
                  console.warn(
                    `Enrichment failed for recall ${row.id}, will retry next run`
                  );
                  return 0;
                }
