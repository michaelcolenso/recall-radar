...
export const makes = sqliteTable(
  ...
  (table) => [index("idx_makes_slug").on(table.slug)]
);
...
export const recalls = sqliteTable(
  ...
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    vehicleYearId: integer("vehicle_year_id")
      .notNull()
      .references(() => vehicleYears.id, { onDelete: "cascade" }),
    ...
    severityLevel: text("severity_level", { enum: severityLevels })
      .notNull()
      .default("UNKNOWN"),
...
// ─── INGESTION LOGS ─────────────────────────────────────────────
export const ingestionLogs = sqliteTable(
  "ingestion_logs",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
  ...
  (table) => [index("idx_logs_type_status").on(table.runType, table.status)]
);
