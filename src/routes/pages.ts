...
pageRoutes.get("/", async (c) => {
  ...
  const html = await getCachedOrRender(
    c.env.PAGE_CACHE,
    "page:home",
    86400,
    async () => {
      const [makesResult, statsResult] = await Promise.all([
        c.env.DB.prepare("SELECT name, slug FROM makes ORDER BY name").all<{
          name: string;
          slug: string;
        }>(),
        Promise.all([
          c.env.DB.prepare("SELECT COUNT(*) as count FROM recalls").first<{
            count: number;
          }>(),
          c.env.DB.prepare(
            "SELECT COUNT(*) as count FROM vehicle_years"
          ).first<{ count: number }>(),
          c.env.DB.prepare("SELECT COUNT(*) as count FROM makes").first<{
            count: number;
          }>(),
        ]),
      ]);
...
pageRoutes.get("/:makeSlug", async (c) => {
  ...
  const make = await c.env.DB.prepare(
    "SELECT id, name, slug FROM makes WHERE slug = ?"
  )
    .bind(makeSlug)
...
pageRoutes.get("/:makeSlug/:modelSlug", async (c) => {
  ...
  const make = await c.env.DB.prepare(
    "SELECT id, name FROM makes WHERE slug = ?"
  )
    .bind(makeSlug)
...
pageRoutes.get("/:makeSlug/:modelSlug/:year", async (c) => {
  ...
  const model = await c.env.DB.prepare(
    "SELECT id, name FROM models WHERE make_id = ? AND slug = ?"
  )
    .bind(make.id, modelSlug)
    .first<{ id: number; name: string }>();
  if (!model) {
  ...
  const html = await getCachedOrRender(
    c.env.PAGE_CACHE,
    `page:year:${makeSlug}:${modelSlug}:${year}`,
    43200,
    async () => {
