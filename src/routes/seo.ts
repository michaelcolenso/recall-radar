...
seoRoutes.get("/sitemap.xml", async (c) => {
  const siteUrl = c.env.SITE_URL;
  const xml = await getCachedOrRender(
    c.env.PAGE_CACHE,
    "sitemap:xml",
    86400,
    async () => {
      const [makesResult, modelsResult, yearsResult] = await Promise.all([
        c.env.DB.prepare("SELECT slug FROM makes ORDER BY slug").all<{
          slug: string;
        }>(),
        c.env.DB.prepare(
      ...
      for (const m of modelsResult.results) {
        urls.push(
          sitemapUrl(
            `${siteUrl}/${m.make_slug}/${m.model_slug}`,
            now,
            "0.7",
            "weekly"
          )
      ...
      for (const y of yearsResult.results) {
        urls.push(
          sitemapUrl(
            `${siteUrl}/${y.make_slug}/${y.model_slug}/${y.year}`,
            now,
            "0.9",
            "weekly"
          )
      ...
      const totalUrls =
        1 +
        makesResult.results.length +
        modelsResult.results.length +
        yearsResult.results.length;

      if (totalUrls > 50000) {
        console.warn(
          `Sitemap has ${totalUrls} URLs — consider splitting into a sitemap index`
        );
      }
