...
seoRoutes.get("/sitemap.xml", async (c) => {
  ...
  if (totalUrls > MAX_URLS_PER_SITEMAP) {
    const { value: indexXml, hit: indexHit } = await getCachedOrRender(c.env.PAGE_CACHE, withSeoCacheVersion("sitemap:index"), 86400, async () => {
      const parts: string[] = [
  ...
  const { value: xml, hit: xmlHit } = await getCachedOrRender(c.env.PAGE_CACHE, withSeoCacheVersion("sitemap:xml"), 86400, async () => {
    ...
    for (const campaign of campaignRows.results) {
      urls.push(
        sitemapUrl(
          `${siteUrl}/recall/${campaign.campaign_number}`,
          campaign.lastmod,
          "0.5",
          "weekly",
        )
...
seoRoutes.get("/sitemap-makes.xml", async (c) => {
  const siteUrl = c.env.SITE_URL;
  const { value: xml, hit } = await getCachedOrRender(c.env.PAGE_CACHE, withSeoCacheVersion("sitemap:makes"), 86400, async () => {
...
seoRoutes.get("/sitemap-models.xml", async (c) => {
  const siteUrl = c.env.SITE_URL;
  const { value: xml, hit } = await getCachedOrRender(c.env.PAGE_CACHE, withSeoCacheVersion("sitemap:models"), 86400, async () => {
...
seoRoutes.get("/sitemap-years-:page{.+\\.xml}", async (c) => {
  ...
  const { value: xml, hit } = await getCachedOrRender(c.env.PAGE_CACHE, withSeoCacheVersion(`sitemap:years:${page}`), 86400, async () => {
...
seoRoutes.get("/sitemap-components-:page{.+\\.xml}", async (c) => {
  ...
  const { value: xml, hit } = await getCachedOrRender(c.env.PAGE_CACHE, withSeoCacheVersion(`sitemap:components:${page}`), 86400, async () => {
...
seoRoutes.get("/sitemap-campaigns-:page{.+\\.xml}", async (c) => {
  ...
  const { value: xml, hit } = await getCachedOrRender(c.env.PAGE_CACHE, withSeoCacheVersion(`sitemap:campaigns:${page}`), 86400, async () => {
...
function getYearRows(db: D1Database, limit?: number, offset?: number): Promise<D1Result<YearSitemapRow>> {
  ...
  return typeof limit === "number"
    ? query.bind(limit, offset ?? 0).all<YearSitemapRow>()
    : query.all<YearSitemapRow>();
}
