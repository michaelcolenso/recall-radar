...
## Ordered first run (do this in order)

1. `npm install`
2. `cp .env.example .env` and replace placeholders
3. `npm run db:push`
4. `npm run ingest -- --make TOYOTA --year-start 2020 --year-end 2021`
5. `npm run enrich -- --make TOYOTA --batch-size 25 --concurrency 3`
6. `npm run dev`

> If steps 3–5 fail, it usually means `.env` still has placeholder credentials or outbound network/database access is unavailable.
...
After the ordered setup, validate:

- `npm run db:push` finishes without Prisma connection errors
- `npm run ingest -- --make TOYOTA --year-start 2020 --year-end 2021` reports non-zero makes/models/recalls
- `npm run enrich -- --make TOYOTA --batch-size 25 --concurrency 3` reports recalls processed
- Homepage stats are non-zero at `http://localhost:3000`
